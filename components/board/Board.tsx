...
export default class BoardView extends Component {
  ...
  constructor(props) {
    ...
    Promise.all([Audio.Sound.createAsync(require('../../assets/move.mp3')), Audio.Sound.createAsync(require('../../assets/capture.mp3'))])
        .then(([moveSound, captureSound]) => {
          console.log("loaded sounds");
          this.setState({sounds: {moveSound: moveSound.sound, captureSound: captureSound.sound}});
        });

    const game = new Chess(props.fen);
    console.log("fen is " + props.fen);

    this.state = {
      game,
      board: this.createBoardData(game, props.fen),
      sounds: {moveSound: null, captureSound: null}
    };
  }

  componentDidUpdate(prevProps) {
    const nextFen = this.props.fen;
    if (nextFen !== prevProps.fen) {
      this.setState({
  ...
  movePiece = (to, from) => {
    ...
    if (moveResult && moveResult.captured) {
      if(this.state.sounds.captureSound){
        console.log("playing capture");
        this.state.sounds.captureSound.setPositionAsync(0); 
        this.state.sounds.captureSound.playAsync(); 
      }
    } else {
      if(this.state.sounds.moveSound){
        this.state.sounds.moveSound.setPositionAsync(0); 
        this.state.sounds.moveSound.playAsync();
      }
    }
  ...
  createBoardData(game, newFen = undefined) {
    if (newFen) {
      game.load(newFen);
    }
